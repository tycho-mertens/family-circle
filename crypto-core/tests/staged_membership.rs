...
fn pair() -> (CryptoCore, CryptoCore, String) {
    ...
    bob.join_from_welcome_with_admin(&alice.create_welcome(&circle, &kp).unwrap(), Some(admin))
        .unwrap();
    (alice, bob, circle)
}

fn restart(core: CryptoCore) -> CryptoCore {
    let saved = core
        .export_encrypted_state(&[42; 32], b"durable outbox and cursor")
        .unwrap();
    CryptoCore::import_encrypted_state(&[42; 32], &saved)
        .unwrap()
        .0
}
...
fn competing_commit_fails_closed_without_losing_the_prepared_state() {
    ...
    bob.adopt_membership_admin(&circle, &alice_id, &bob_id)
        .unwrap(); // modified hostile client
...
fn creator_can_leave_and_successor_can_manage_the_remaining_circle() {
    ...
    // The outgoing admin's MLS-authenticated transfer is processed before
    // departure, so the successor is the only permitted committer.
    alice
