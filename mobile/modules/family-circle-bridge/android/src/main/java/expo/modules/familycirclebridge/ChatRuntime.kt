...
/** The local checkpoint is separate from seed backups and Android Auto Backup.
 * One atomic file contains encrypted MLS/app state and the encrypted location
 * vault. No envelope may be uploaded until its state transaction commits.
 * LocationRuntime uses this same monitor, including from the native service.
 */
object ChatRuntime {
  val lock = Any()
  ...
  fun read(context: Context): ByteArray? = synchronized(lock) {
  ...
  fun locationVault(context: Context): String? = synchronized(lock) {
    val target = AtomicFile(File(context.noBackupFilesDir, "chat-state-v1.json"))
    if (!target.baseFile.exists()) return@synchronized null
    JSONObject(target.readFully().toString(Charsets.UTF_8)).optString("locations").takeIf { it.isNotEmpty() }
  }

  fun configure(context: Context, encKey: ByteArray, appMetadata: ByteArray, preserveLocations: Boolean) = synchronized(lock) {
  ...
  fun commit(appMetadata: ByteArray): ByteArray = synchronized(lock) {
    check(active)
  ...
  /** Native location changes also consume MLS ratchets. Save those before
   * returning a control envelope to JS, using the last committed app metadata.
   */
  fun checkpointLocation() = synchronized(lock) {
    check(!failed && key != null) { "Chat storage is not ready" }
    if (!active) {
      try { persist(metadata) } catch (error: Exception) { failed = true; throw error }
    }
  }

  fun checkLocationCommand(op: String) = synchronized(lock) {
    check(!failed && key != null) { "Chat storage is not ready" }
    // Only incoming controls are executed inside a JS chat transaction.
    if (op == "start" || op == "reconcile") {
      val circles = JSONObject(metadata.toString(Charsets.UTF_8)).optJSONArray("circles")
      if (circles != null) for (index in 0 until circles.length()) {
        check(circles.getJSONObject(index).optString("pendingCommitEventId").isEmpty()) { "Circle sync is busy; try again shortly" }
        check(circles.getJSONObject(index).optString("role", "member") != "member" || !circles.getJSONObject(index).optBoolean("recoveryRequired")) { "Reconnect your Circles before starting location sharing" }
      }
    }
    check(!active || op == "control" || op == "stop") { "Circle sync is busy; try again shortly" }
  }
